...
# 🚨 ИСПРАВЛЯЕМ USER TOKEN ERROR
echo "🚨 ИСПРАВЛЯЕМ USER TOKEN ERROR"
echo "============================="

# Переходим в правильную папку
cd /opt/sofiya

# 1. ПРОВЕРЯЕМ AUTH STORE
echo "1️⃣ Проверяем authStore..."
cat frontend/store/authStore.ts

# 2. ИСПРАВЛЯЕМ SETTINGS PANEL - ИСПОЛЬЗУЕМ ПРАВИЛЬНОЕ ПОЛЕ
echo "2️⃣ Исправляем SettingsPanel - используем правильное поле..."
...
echo ""
echo "🎯 USER TOKEN ERROR ИСПРАВЛЕН!"
echo "✅ user?.token заменен на user?.access_token || user?.token"
