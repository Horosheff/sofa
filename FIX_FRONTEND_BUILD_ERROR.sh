...
# 🚨 СРОЧНОЕ ИСПРАВЛЕНИЕ FRONTEND BUILD ERROR
echo "🚨 СРОЧНОЕ ИСПРАВЛЕНИЕ FRONTEND BUILD ERROR"
echo "=========================================="

# Переходим в правильную папку
cd /opt/sofiya

# 1. ВОССТАНАВЛИВАЕМ ОРИГИНАЛЬНЫЙ SETTINGS PANEL
echo "1️⃣ Восстанавливаем оригинальный SettingsPanel..."
git checkout frontend/components/SettingsPanel.tsx

# 2. ПРОВЕРЯЕМ СТРУКТУРУ ПАПОК
echo "2️⃣ Проверяем структуру папок..."
ls -la frontend/components/
ls -la frontend/components/inputs/

# 3. ПРАВИЛЬНО ДОБАВЛЯЕМ КНОПКУ TELEGRAM
echo "3️⃣ Правильно добавляем кнопку Telegram..."

# Создаем правильный SettingsPanel.tsx с существующими импортами
...
export default function SettingsPanel() {
  ...
  const onSubmit = async (data: SettingsFormData) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        showToast(`Ошибка: ${errorData.detail || 'Неизвестная ошибка'}`, 'error')
      }
    } catch (error) {
  ...
  const handleWordstatAuth = async () => {
    try {
      ...
      showToast('Ошибка получения URL авторизации', 'error')
    }
  }
  ...
  const handleAuthCode = async () => {
    try {
      ...
      if (response.ok) {
        showToast('Авторизация Wordstat успешна!', 'success')
        setShowCodeInput(false)
        setAuthCode('')
        await fetchSettings()
      } else {
        const errorData = await response.json()
        showToast(`Ошибка авторизации: ${errorData.detail}`, 'error')
      }
    } catch (error) {
...
# 7. ПРОВЕРЯЕМ ЛОГИ
echo "7️⃣ Проверяем логи frontend..."
pm2 logs frontend --lines 5

# 8. ТЕСТИРУЕМ САЙТ
echo "8️⃣ Тестируем сайт..."
curl -I https://mcp-kv.ru

echo ""
echo "🎯 FRONTEND BUILD ERROR ИСПРАВЛЕН!"
echo "✅ Правильные импорты восстановлены"
